...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ecco="http://owl.cs.manchester.ac.uk/diff">
		...
		<html>
			...
			<body>
				...
				<form action="getXml" method="post" class="inl">
					[<a id="top3" target="_blank"><xsl:attribute name="href">./getXml?uuid=<xsl:value-of select="$uuid"/></xsl:attribute>Source XML</a>]
				</form><br/><br/>
				...
				<form action="output" method="post" name="GenSymTrigger">
					...
					<input type="submit" name="gensyms" value="Use Labels"/><xsl:text> </xsl:text>
					<input type="submit" name="gensyms" value="Use Term Names"/><xsl:text> </xsl:text>
					<input type="submit" name="gensyms" value="Use GenSyms"/>
				</form>
							...
							<ul id="tree1">
								...
								<ul>
									...
									<xsl:if test="$rprospred > 0">
										<li><xsl:text>  </xsl:text>
											<input type="checkbox" name="ineffRems" onClick="toggleDiv('ravred','rnpred');"/>
											<xsl:text>  </xsl:text>
											...
											<ul style="display:none">
												...
												<xsl:if test="$rprospnewred > 0">
													<li id="rnpredtrig">
														<img src="images/blank.png" alt=""></img><xsl:text>  </xsl:text>
														<input type="checkbox" name="ineffRems" onClick="toggleDiv('rnpred');"/>
														<xsl:text>  </xsl:text><a href="#rnpred">New (<xsl:value-of select="$rprospnewred"/>)</a><b id="t1"/><xsl:text>  </xsl:text>
														<img src="images/info_bubble.png" alt="" align="right" width="14" height="14" class="hotspot" onmouseout="tooltip.hide();"
							...
							<ul id="tree2">
								...
								<ul>
									...
									<xsl:if test="$addedprospred > 0">
										<li><xsl:text>  </xsl:text>
											<input type="checkbox" name="ineffAdds" onClick="toggleDiv('aavred','anpred');"/>
											<xsl:text>  </xsl:text>
											...
											<ul style="display:none">
												...
												<xsl:if test="$aprospnewred > 0">
													<li id="anpredtrig">
														<img src="images/blank.png" alt=""></img><xsl:text>  </xsl:text>
														<input type="checkbox" name="ineffAdds" onClick="toggleDiv('anpred');"/>
														<xsl:text>  </xsl:text><a href="#anpred">New (<xsl:value-of select="$aprospnewred"/>)</a><xsl:text>  </xsl:text>
														<img src="images/info_bubble.png" alt="" align="right" width="14" height="14" class="hotspot" onmouseout="tooltip.hide();"
			...
			<tbody id="st" style="display:none">
				...
				<tr>
					...
					<th class="addition" onmouseout="tooltip.hide();"
						onmouseover="tooltip.show('Classes that have gained superclasses');">Classes <xsl:text disable-output-escaping="yes">&amp;darr;</xsl:text></th>
					<th class="addition" onmouseout="tooltip.hide();"
						onmouseover="tooltip.show('Classes that have gained subclasses');">Classes <xsl:text disable-output-escaping="yes">&amp;uarr;</xsl:text></th>
				</tr>
			...
			<tbody id="weak" style="display:none">
				...
				<tr>
					...
					<th class="removal" onmouseout="tooltip.hide();"
						onmouseover="tooltip.show('Classes that have lost superclasses');">Classes <xsl:text disable-output-escaping="yes">&amp;uarr;</xsl:text></th>
					<th class="removal" onmouseout="tooltip.hide();"
						onmouseover="tooltip.show('Classes that have lost subclasses');">Classes <xsl:text disable-output-escaping="yes">&amp;darr;</xsl:text></th>
				</tr>
	...
	<xsl:template match="AddedNewProspectiveRedundancy">
		<xsl:if test="child::node()">
			<tbody id="anpred" style="display:none">
				<tr class="withoutstyle">
	...
	<xsl:template match="RemovedNewProspectiveRedundancy">
		<xsl:if test="child::node()">
			<tbody id="rnpred" style="display:none">
				<tr class="withoutstyle">
	...
	<xsl:template match="Change" mode="OneColumnEffectualAddition">
		<tr class="addition">
			...
			<td>
				<xsl:call-template name="getConceptChanges">
					<xsl:with-param name="mode" select="ConceptChanges/DirectChanges/Specialisation"/>
				</xsl:call-template>
			</td>
			<td>
				<xsl:call-template name="getConceptChanges">
					<xsl:with-param name="mode" select="ConceptChanges/DirectChanges/Generalisation"/>
				</xsl:call-template>
			</td>
		</tr>
	</xsl:template>
	...
	<!-- Get concept changes and their witnesses -->
	<xsl:template name="getConceptChanges">
		<xsl:param name="mode"/>
